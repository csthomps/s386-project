...
st.text("- homewins-awaywins: the number of teams the home game has won so far in the season - the away team's win count")
st.text("- home_win_prob and predicted_diff: these are calculated using through Glickman and Stern's method (which I talk about in my blog posts) and all historical data")
st.text("- home_win_prob_last15 and predicted_diff_last15: these are calculated using only the last 15 weeks of data")
st.text("- q1_differential, halftime_differential, q3_differential, final_differential: the score differential (home score - away score) at each point in the game")
